...
const WIDTH: u32 = 64;
const HEIGHT: u32 = 64;
const OUTPUT_PATH: &str = "target/examples/pgm_ascii_roundtrip.pgm";
...
fn main() -> kimage::Result<()> {
    ...
    pgm::encode_ascii(&mut file, image)?;
    drop(file);

    let file = File::open(path)?;
    let decoded = pgm::decode_ascii(&mut BufReader::new(file))?;
    ...
    println!("ASCII PGM roundtrip succeeded: {}", path.display());
