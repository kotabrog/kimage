...
use kimage::{
    ...
    codecs::pnm::{self, PnmEncodeFormat},
};

const WIDTH: u32 = 64;
const HEIGHT: u32 = 64;
const OUTPUT_PATH: &str = "target/examples/pnm_roundtrip.ppm";
...
fn main() -> kimage::Result<()> {
    ...
    pnm::encode(&mut file, image, PnmEncodeFormat::PpmBinary)?;
    drop(file);

    let file = File::open(path)?;
    let decoded = pnm::decode(&mut BufReader::new(file))?;
    ...
    println!("PNM roundtrip succeeded: {}", path.display());
