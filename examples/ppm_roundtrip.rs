...
const WIDTH: u32 = 64;
const HEIGHT: u32 = 64;
const OUTPUT_PATH: &str = "target/examples/ppm_roundtrip.ppm";
...
fn main() -> kimage::Result<()> {
    ...
    ppm::encode(&mut file, image)?;
    drop(file);

    let file = File::open(path)?;
    let decoded = ppm::decode(&mut BufReader::new(file))?;
    ...
    println!("PPM roundtrip succeeded: {}", path.display());
