...
const WIDTH: u32 = 64;
const HEIGHT: u32 = 64;
const OUTPUT_PATH: &str = "target/examples/pbm_ascii_roundtrip.pbm";
...
fn main() -> kimage::Result<()> {
    ...
    pbm::encode_ascii(&mut file, image)?;
    drop(file);

    let file = File::open(path)?;
    let decoded = pbm::decode_ascii(&mut BufReader::new(file))?;
    ...
    println!("ASCII PBM roundtrip succeeded: {}", path.display());
